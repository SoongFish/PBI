...
def Load_File():
    try:
        if flag_login == 0 or flag_login == None or flag_login == '':
            ...
            bt_test_yearly = tk.Button(frame_word, text = '워드클라우드', overrelief = 'solid', command = lambda:Makegraph_Wordcloud(target_word.get()))
            bt_test_yearly.place(x = 10, y = 40)
            ...
            bt_test_yearly = tk.Button(frame_kmeans, text = 'K-means', overrelief = 'solid', command = lambda:Kmeans(target_Kmeans.get(), cluster.get()))
            bt_test_yearly.place(x = 10, y = 40)
...
def Makegraph_Wordcloud(target):
    try:
        if flag_login == 0 or flag_login == None or flag_login == '':
            Login()
        #elif flag_prepro == 0:
            ...
            data_wordcloud = pd.DataFrame(data_origin[target], columns = ['contents'])
            data_wordcloud['contents'] = data_origin[target].apply(lambda x: re.sub('[^가-힣]',' ', x))
            ...
            plt.axis('off')
            #plt.show()
            plt.savefig(resultdir + filename_dateflag + target + ' - wordcloud.png', dpi = 100)
